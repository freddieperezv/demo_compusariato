...
def create_qualifying_properties(node, name=None, etsi='etsi'):
    ...
    qualifying = etree.SubElement(
        obj_node, etree.QName(EtsiNS, 'QualifyingProperties'),
        nsmap={etsi: EtsiNS}
    )
...
def add_data_object_format(
        node, reference, description=None,
...
class Xades(object):
    ...
    def sign(self,xmldata, key, passphrase):
        get_unique_id = lambda : "id-{}".format(uuid.uuid4())
        root = etree.parse(BytesIO(xmldata.encode('utf-8'))).getroot()
        signature_id = get_unique_id()
        signature = xmlsig.template.create(
            xmlsig.constants.TransformInclC14N,
            xmlsig.constants.TransformRsaSha1, 'Signature'
        )
        ...
        qualifying = create_qualifying_properties(
            signature, name=get_unique_id()
        )
        create_signed_properties(qualifying, name=signature_id)
        add_data_object_format(
            qualifying, get_unique_id(), 'contenido comprobante', None,
            'text/xml'
        )
