...
class AccountPayment(models.Model):
    ...
    def onchange_date_to(self):
        if self.date_to and self.date_to > self.date:
    ...
    def _compute_check_amount_in_words(self):
        ...
        for pay in self:
            if pay.currency_id:
                pay.check_amount_in_words = amount_to_text_es.amount_to_text(pay.amount)
            else:
    ...
    def _get_report_base_filename(self):
            self.ensure_one()
            return 'Cheque-%s' % (self.name)

    # def action_post(self):
    #     for rec in self:
    #         super(AccountPayment,rec).action_post()
    #         account_check = rec.env['account.cheque']
    #         if rec.payment_method_id.code in ['check_printing','batch_payment'] and not rec.is_internal_transfer:
    #             types = 'outgoing'
    ...
    #                 credit = rec.journal_id.default_account_id.id
    #                 date_check = rec.date
    #                 bank_account = rec.journal_id.default_account_id.id
    ...
    #                 debit = rec.journal_id.default_account_id.id
    ...
    #                 move.account_cheque_id = check_id.id
